...
function renderShopInfo() {
  ...
  document.getElementById('shop-copyright').textContent = `© ${new Date().getFullYear()} ${shopInfo.name} 🕊 Nopsa ツ`;
  if(shopInfo.hero) {
...
function setupSocialLinks() {
  ...
  if (contact.instagram) {
    ...
    if (ttLink && contact.tiktok) {
      const ttUsername = contact.tiktok.replace('@', '');
      ttLink.href = `https://www.tiktok.com/@${ttUsername}`;
...
// Fungsi untuk mendapatkan ikon kategori berdasarkan ID
  ...
  const iconMap = {
    'unit': 'foto/icon/mobil.png',
    'rumah': 'foto/icon/rumah.png',
  };

  return iconMap[categoryId] || 'foto/icon/default.png';
};
  ...
  categoriesToShow.forEach(category => {
    ...
    const iconElement = categoryCard.querySelector('.category-icon');
iconElement.src = getCategoryIcon(category.id);
...
function renderProducts(categoryId = 'all') {
  ...
  if (categoryId === 'all') {
    categoryTitle.textContent = 'Semua';
  } else {
